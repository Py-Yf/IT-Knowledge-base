...
## 2. web3 模块加载
### 2.1 首先需要将web3模块安装在项目中
```shell
npm install web3@0.20.1
```
### 2.2 创建web3实例
然后创建一个web3实例，设置一个“provider”；为了保证我们的MetaMask设置好的provider不被覆盖掉，在引入web3之前我们一般要做当前环境检查(以v0.20.1为例)
```
...
```
## 3. 异步回调(callback)
web3js API 设计的最初目的，主要是为了和本地 RPC 节点共同使用，所以默认情况下发送的是同步 HTTP 请求；如果要发送异步请求，可以在函数的最后一个参数位置上，传入一个回调 函数。回调函数是可选(optioanl)的；我们一般采用的回调风格是所谓的“错误优先”，例如:
```js
...
为了帮助 web3 集成到不同标准的所有类型项目中，1.0.0 版本提供了多种方式来处理异步函数。 大多数的 web3 对象允许将一个回调函数作为最后一个函数参数传入，同时会返回一个promise 用于链式函数调用。<br\>
以太坊作为一个区块链系统，一次请求具有不同的结束阶段。为了满足这样的要求，1.0.0 版本 将这类函数调用的返回值包成一个“承诺事件”(promiEvent)，这是一个 promise 和 EventEmitter 的结合体；PromiEvent 的用法就像 promise 一样，另外还加入了.on，.once 和.off方法。
```js
...
批处理请求允许我们将请求排序，然后一起处理它们; 批处理请求主要用于确保请求的顺序，并串行处理。
...
```
## 7. 大数处理(big numbers)
JavaScript中默认的数字精度较小，所以web3.js会自动添加一个依 赖库 BigNumber，专门用于大数处理；对于数值，我们应该习惯把它转换成BigNumber对象来处理如：
```js
...
```
BigNumber.toString(10)对小数只保留20位浮点精度。所以推荐的做 法是，我们内部总是用 wei 来表示余额(大整数)，只有在需要显示 给用户看的时候才转换为ether或其它单位
## 8. 常用 API
### 8.1 基本信息查询
#### 查看 web3 版本
...
#### 区块查询
* 同步:web3.eth.getBloock(hashStringOrBlockNumber, [returnTransactionObjects] )
